...
if(!is.null(input$psa) & !check2) {
  ...
  for(i in 1:length(input$psa)){
    ...
    if(input$psa[i] == "Info-rank" & !is.null(tryCatch(make_info_rank()$rank, error = function(cond) NULL))) iIR <- TRUE
    if(input$psa[i] == "EVPPI" & !is.null(tryCatch(compute_evppi()$evppi, error = function(cond) NULL))) iEVPPI <- TRUE
  }
}
```
