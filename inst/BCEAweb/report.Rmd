...
any_data <- !all(is.null(param()), is.null(param2()))

# If 0 then no economic analysis executed
executed_analysis <- input$buttonsum != 0

if(any_data & !is.null(input$assumption)) iAssumption <- TRUE

if(executed_analysis && !is.null(input$bcea)) {
  iEcEval <- TRUE
...
if (executed_analysis && !is.null(input$psa)) {
  iPSA <- TRUE
