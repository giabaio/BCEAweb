...
#' @name Smoking
#' @docType data
#' @aliases Smoking cost data eff life.years pi_post smoking smoking_output treats smoking_parameters smoking_results
#' @format The Smoking example is included using 3 object:
#' \describe{
#' \item{`Smoking.RData`}{
...
#' \item{`data`}{a dataset containing the characteristics of the smokers
#'  in the UK population}
#' \item{`eff`}{a matrix of 500 simulations from the
#'  posterior distribution of the clinical benefits associated with the four
#'  strategies}
#' \item{`life.years`}{a matrix of 500 simulations from the
...
#' \item{`smoking`}{a data frame containing the inputs needed for the
...
#' \item{`smoking_mat`}{a matrix obtained by running the network
#'  meta-analysis model based on the data contained in the `smoking` object}
#' \item{`treats`}{a vector of labels associated with the four strategies}
#' }}
#' \item{`smoking_parameters.csv`}{A `csv` file including the (named) `pi_post`
#'  variable describing above in a format that can be to the web-app using the Spreadsheet option}
#' \item{`smoking_results.csv`}{A `csv` file including the (named) effectiveness 
...
#' vaccine_parameters vaccine_results
#' @format The influenza vaccination example is included using 3 objects:
#' \describe{
#' \item{`Vaccine.Rdata`}{A data list including the variables needed for the influenza
#' vaccination. The variables are as follows:
#' \describe{
#'   \item{`cost`}{a matrix of simulations from the posterior
#'   distribution of the overall costs associated with the two treatments}
#'   \item{`c.pts`}{}
#'   \item{`cost.GP`}{a matrix of simulations from the posterior
#'   distribution of the costs for GP visits associated with the two treatments}
#'   \item{`cost.hosp`}{a matrix of simulations from the posterior
#'   distribution of the costs for hospitalisations associated with the two
#'   treatments}
#'   \item{`cost.otc`}{a matrix of simulations from the posterior distribution
#'   of the costs for over-the-counter medications associated with the two treatments}
#'   \item{`cost.time.off`}{a matrix of simulations from the posterior
#'   distribution of the costs for time off work associated with the two treatments}
#'   \item{`cost.time.vac`}{a matrix of simulations from the posterior
#'   distribution of the costs for time needed to get the vaccination associated
#'   with the two treatments}
#'   \item{`cost.travel`}{a matrix of simulations from the posterior
#'   distribution of the costs for travel to get vaccination associated with the
#'   two treatments}
#'   \item{`cost.trt1`}{a matrix of simulations from the
#'   posterior distribution of the overall costs for first line of treatment
#'   associated with the two interventions}
#'   \item{`cost.trt2`}{a matrix of simulations from the posterior distribution
#'   of the overall costs for second line of treatment associated with the two
#'   interventions}
#'   \item{`cost.vac`}{a matrix of simulations from the posterior
#'   distribution of the costs for vaccination}
#'   \item{`eff`}{a matrix of simulations from the posterior distribution of
...
#'   \item{`QALYs.adv`}{a vector from the posterior distribution of the
#'   QALYs associated with advert events}
#'   \item{`QALYs.death`}{a vector from the posterior distribution of the
#'   QALYs associated with death}
#'   \item{`QALYs.hosp`}{a vector from the posterior distribution of the
#'   QALYs associated with hospitalisation}
#'   \item{`QALYs.inf`}{a vector from the posterior distribution of the
#'   QALYs associated with influenza infection}
#'   \item{`QALYs.pne`}{a vector from the posterior distribution of the
#'   QALYs associated with pneumonia}
#'   \item{`treats`}{a vector of labels associated with the two treatments}
#'   \item{`vaccine_mat`}{a matrix containing the simulations for the
#'   parameters used in the original model}
#'   }}
#' \item{`vaccine_parameters.csv`}{A `csv` file including the (named) input parameters
#'  for the vaccination example in a format that can be to the web-app using the Spreadsheet option}
#' \item{`vaccine_results.csv`}{A `csv` file including the (named) effectiveness 
#'  and costs results in a format that can be uploaded to the web-app using the Spreadsheet option}
#' }
